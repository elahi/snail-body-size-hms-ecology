################################################################################
##' @title Extract model summaries, Tegula funebralis (x1p; era only model, pooled)
##' @author Robin Elahi
...
model_path <- "brm_models/chfu/logsize_x12p/"
...
extract_coefs <- function(fit, truncate_logic = FALSE, my_quantile = NA){
  ...
  levels(coefs$variable) <- c("Era", "Sample Area", 
                              "Era x Sample Area")
  
  # Relevel the variables
  coefs <- coefs %>%
    mutate(variable = fct_relevel(variable, "Era", "Sample Area", 
                                  "Era x Sample Area"))
...
## Extract effects of era
fit_draws_spread <- fit %>% 
  spread_draws(b_Intercept, b_erapresent, b_sampleAreaWara.D, 
               `b_erapresent:sampleAreaWara.D`) %>% 
  mutate(
    # size_past_B = b_Intercept,
    # size_past_D = b_Intercept + b_sampleAreaWara.D,
    # size_present_B = b_Intercept + b_erapresent,
    # size_present_D = b_Intercept + b_erapresent + b_sampleAreaWara.D +
    #   `b_erapresent:sampleAreaWara.D`, 
    era_B = b_erapresent, 
    era_D = b_erapresent + `b_erapresent:sampleAreaWara.D`
  ) %>% 
  select(-c(b_Intercept, b_erapresent, b_sampleAreaWara.D, 
            `b_erapresent:sampleAreaWara.D`))

fit_draws_spread_long <- fit_draws_spread %>%
  gather(key = .variable, value = .value, c(era_B, era_D)) %>% 
  group_by(.variable)

era_df <- fit_draws_spread_long %>%
  median_qi(.width = c(0.95, 0.8)) %>% 
  mutate(variable = as.factor(.variable), 
         truncate = FALSE, 
         quantile = NA)
...
for(i in 1:(nFiles-1)){
  ...
  ##### Extract model coefficients #####
  coefs <- extract_coefs(fit = fit, truncate_logic = TRUE, 
                         my_quantile = my_quantile)
  ...
  era_df_i <- fit_draws_spread_long %>%
    median_qi(.width = c(0.95, 0.8)) %>% 
    mutate(variable = as.factor(.variable), 
           truncate = TRUE, 
           quantile = my_quantile)
  
  era_df <- rbind(era_df, era_df_i)
...
write.csv(era_df, file = paste(model_path, "model_summaries/", "era_df.csv", sep = ""))
