################################################################################
##' @title Extract model summaries, Lottia digitalis (x1p; era only model, pooled)
##' @author Robin Elahi
...
model_path <- "brm_models/lodi/logsize_x1p/"
...
fit <- b_final
coefs_df <- extract_coefs(fit = fit, truncate_logic = FALSE, my_quantile = NA, x123 = FALSE)
...
for(i in 1:(nFiles-1)){
  ...
  ##### Extract model coefficients #####
  coefs <- extract_coefs(fit = fit, truncate_logic = TRUE, my_quantile = my_quantile, x123 = FALSE)
