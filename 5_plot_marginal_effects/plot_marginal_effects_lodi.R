################################################################################
##' @title Plot marginal effects, Lottia digitalis
##' @author Robin Elahi
...
## Truncate data
statDat <- truncate_data(dat = hexDF, era = "past", quant = my_p, filter_data = TRUE)
...
## Save as new object
tide_offset <- 0.025
pi_lodi_tide <- d_new_pi %>% 
  mutate(tideHTm_offset = ifelse(era == "past", tideHTm,
                                 tideHTm + tide_offset), 
         species = "Lottia digitalis", 
