...
# save(dat, file = "output/monterey_tides_2015.RData")

load("output/monterey_tides_2015.RData")
glimpse(dat)
str(dat)
names(dat)
...
new_df <- data.frame(tidal_height = seq(-0.5, 2, by = 0.01), 
                     Station = "Monterey")
head(new_df)
str(new_df)
...
test3 %>%
  ggplot(aes(tidal_height, air_exposure_proportion)) +
  geom_point()
